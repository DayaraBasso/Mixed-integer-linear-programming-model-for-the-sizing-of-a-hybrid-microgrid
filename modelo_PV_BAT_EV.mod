...
subject to restricao28 {j in Q, s in S}:
	SOC[j] = SOC_a[j] + sum {h in H} Pcarga[h,s] * eta_c_VE; 
...
subject to restricao38 {s in S, h in H: h == 24}:
...
subject to restricao39 {h in H,s in S}:
	z * P_pv_m[h,s] + P_BAT_d[h,s] >=  P_BAT_c[h,s] + Pcarga[h,s];
...
subject to restricao_energia_minima {s in S}:
  SOC_inicial-SOC_min +
    eta_c_VE*sum{h in H} (z * P_pv_m[h,s] + P_BAT_d[h,s]-P_BAT_c[h,s])
